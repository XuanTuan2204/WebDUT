...
namespace WebsiteDUT.Areas.Admin.Controllers
{
    public class QuangCaosController : BaseController
    {
        ...
        public ActionResult Edit(QuangCao quangCao, HttpPostedFileBase editImage)
        {
            if (ModelState.IsValid)
            {
                QuangCao modifyQuangCao = db.QuangCaos.Find(quangCao.MaQuangCao);
                if (modifyQuangCao != null)
                {
                    if (editImage != null && editImage.ContentLength > 0)
                    {
                        modifyQuangCao.AnhDaiDien = new byte[editImage.ContentLength]; // image stored in binary fomate 
                        editImage.InputStream.Read(modifyQuangCao.AnhDaiDien, 0, editImage.ContentLength);
                ...
                db.Entry(modifyQuangCao).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(quangCao);
