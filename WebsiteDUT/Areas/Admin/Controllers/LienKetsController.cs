...
namespace WebsiteDUT.Areas.Admin.Controllers
{
    public class LienKetsController : BaseController
    {
        ...
        public ActionResult Edit(LienKet lienKet, HttpPostedFileBase editImage)
        {
            ...
            if (ModelState.IsValid)
            {
                LienKet modifyLienKet = db.LienKets.Find(lienKet.MaLienKet);
                if (modifyLienKet != null)
                {
                    if (editImage != null && editImage.ContentLength > 0)
                    {
                        modifyLienKet.AnhDaiDien = new byte[editImage.ContentLength]; // image stored in binary fomate 
                        editImage.InputStream.Read(modifyLienKet.AnhDaiDien, 0, editImage.ContentLength);
                ...
                db.Entry(modifyLienKet).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(lienKet);
