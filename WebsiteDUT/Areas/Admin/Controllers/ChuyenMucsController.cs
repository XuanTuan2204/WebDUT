...
namespace WebsiteDUT.Areas.Admin.Controllers
{
    public class ChuyenMucsController : BaseController
    {
        ...
        public ActionResult Edit(ChuyenMuc chuyenMuc, HttpPostedFileBase editImage)
        {
            if (ModelState.IsValid)
            {
                ChuyenMuc modifyChuyenMuc = db.ChuyenMucs.Find(chuyenMuc.MaChuyenMuc);
                if (modifyChuyenMuc != null)
                {
                    if (editImage != null && editImage.ContentLength > 0)
                    {
                        modifyChuyenMuc.AnhDaiDien = new byte[editImage.ContentLength]; // image stored in binary fomate 
                        editImage.InputStream.Read(modifyChuyenMuc.AnhDaiDien, 0, editImage.ContentLength);
                ...
                db.Entry(modifyChuyenMuc).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
